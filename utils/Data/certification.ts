export const certifications = [
  {
    id: 1,
    title: "JavaScript Certification",
    issuer: "CancanIT",
    image: "/certifications/js-cert.png", // <- remove './'
    details: [
      "Proves knowledge of JavaScript language basics, including variables, arrays, objects, expressions, operators, and statements.",
      "Covers functions, classes, DOM manipulation, events handling, and regular expressions with a practical on-page JS task.",
      "Demonstrates ability to build real-world, functional JavaScript applications with a focus on maintainability and problem-solving."
    ],
    skills: ["JavaScript", "Functions", "Classes", "Events", "Regular Expressions"]
  },
  {
    id: 2,
    title: "PCAP – Certified Associate in Python Programming",
    issuer: "OpenEDG Python Institute",
    image: "/certifications/python-cert.png",
    details: [
      "Demonstrates proficiency in Python programming fundamentals, including variables, loops, conditional execution, and basic data structures.",
      "Covers object-oriented programming concepts, functions, and coding techniques for real-world Python applications.",
      "Validates ability to complete coding tasks, understand Python 3 syntax and semantics, and build maintainable Python code."
    ],
    skills: ["Python", "Object-oriented Programming", "Algorithmic Thinking", "Functions", "Loops"]
  },
  {
    id: 3,
    title: "IBM Back-end JavaScript Developer Professional Certificate",
    issuer: "IBM",
    image: "/certifications/backend-cert.png",
    details: [
      "Covers software engineering essentials, Node.js & Express server-side development, asynchronous programming, and CI/CD methodologies.",
      "Includes hands-on experience with Docker, Kubernetes, microservices, NoSQL databases, and cloud-native back-end applications.",
    ],
    skills: ["JavaScript", "Node.js", "Express", "Back-end Development", "Docker", "CI/CD"]
  }
];